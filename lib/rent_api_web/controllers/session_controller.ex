defmodule RentApiWeb.SessionController do
  use RentApiWeb, :controller

  alias RentApi.Accounts
  alias RentApi.Auth.Guardian
  ...
  def create(conn, params) do
    case authenticate(params) do
      ...
      :error ->
        ...
        |> render("error.json", errors: ["User or email invalid"])
    end
  end
  ...
  def refresh(conn, _params) do
    ...
    case Guardian.refresh(jwt, ttl: {30, :days}) do
      ...
      {:error, _reason} ->
        conn
        |> put_status(:unauthorized)
        |> render("error.json", errors: ["Not Authenticated"])
    end
  end
